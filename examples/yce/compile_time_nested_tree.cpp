  ...
  for (int b = 0; b < kBuckets; ++b) {
    ...
    for (int k = 0; k < kPerBucket; ++k) {
      ...
      for (int p = 0; p < prime_count && rem > 1; ++p) {
        ...
        if (exp > 0)
          ne.factors[ne.factor_count++] = {prime, exp};
      }
    }
  }
...
constexpr const number_entry *find_number(const factor_tree &t, int n) {
  ...
  for (int i = 0; i < bucket.populated; ++i) {
    if (bucket.numbers[i].n == n)
      return &bucket.numbers[i];
  }
  return nullptr;
}
...
constexpr int recompose(const number_entry &ne) {
  ...
  for (int i = 0; i < ne.factor_count; ++i) {
    for (int e = 0; e < ne.factors[i].exponent; ++e)
      v *= ne.factors[i].prime;
  }
  return v;
}
...
int main() {
  ...
  static_assert(twelve->factors[0].prime == 2
                && twelve->factors[0].exponent == 2);
  static_assert(twelve->factors[1].prime == 3
                && twelve->factors[1].exponent == 1);
  ...
  for (const auto &bucket : tree.buckets) {
    if (bucket.populated == 0)
      continue;
    std::cout << "  decade " << bucket.decade_start << ".."
              << bucket.decade_start + 9 << " (" << bucket.populated
              << " numbers):\n";
    for (int i = 0; i < bucket.populated; ++i) {
      const number_entry &ne = bucket.numbers[i];
      std::cout << "    " << ne.n << " = ";
      if (ne.factor_count == 0) {
        std::cout << "?\n";
        continue;
      }
