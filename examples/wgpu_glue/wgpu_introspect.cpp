...
{
    if (!from_main_header(decl)) return;
    ...
    if (!clang_equalCursors(decl, clang_getCursorDefinition(decl))) return;
    ...
    if (rec.size_bytes  < 0) rec.size_bytes  = -1;
    if (rec.align_bytes < 0) rec.align_bytes = -1;
    ...
    clang_Type_visitFields(
        ...
        [](CXCursor field, CXClientData ud) {
            ...
            if (f.offset_bits < 0) f.offset_bits = -1;
...
CXChildVisitResult tu_visitor(CXCursor c, CXCursor /*parent*/, CXClientData ud)
{
    auto &tree = *static_cast<api_tree *>(ud);
    switch (clang_getCursorKind(c)) {
        case CXCursor_StructDecl:    visit_struct(c, tree);   break;
        case CXCursor_EnumDecl:      visit_enum(c, tree);     break;
        case CXCursor_TypedefDecl:   visit_typedef(c, tree);  break;
        case CXCursor_FunctionDecl:  visit_function(c, tree); break;
        default: break;
    }
    return CXChildVisit_Continue;
}
...
struct py_type_map {
    ...
    std::string lookup(std::string s) const
    {
        ...
        while (!s.empty() && s.front() == ' ') s.erase(s.begin(), s.begin() + 1);
        ...
        if (!s.empty() && s.back() == '*') return "c_void_p";
        ...
        if (auto it = base.find(s); it != base.end()) return it->second;

        if (auto it = typedef_to_ctypes.find(s); it != typedef_to_ctypes.end())
            return it->second;

        if (enum_names.count(s))   return s;   // IntEnum classes are c-int-shaped
        if (record_names.count(s)) return s;   // Structure subclass
...
{
    py_type_map m;
    for (const auto &e : tree.enums)   m.enum_names.insert(e.name);
    for (const auto &r : tree.records) m.record_names.insert(r.name);
    ...
    while (progress) {
        ...
        for (const auto &t : tree.typedefs) {
            if (m.typedef_to_ctypes.count(t.name)) continue;
...
void emit_python(const api_tree &tree, const std::string &path)
{
    ...
    out <<
        "# Generated by wgpu_introspect. Do not edit by hand.\n"
    ...
    for (std::size_t i = 0; i < tree.records.size(); ++i) {
        for (const auto &f : tree.records[i].fields) {
            ...
            if (f.type_spelling.find('*') != std::string::npos) continue;
    ...
    std::vector<int> mark(tree.records.size(), 0); // 0=unseen 1=in-progress 2=done
    auto visit = [&](auto &self, std::size_t i) -> void {
        if (mark[i] == 2) return;
        if (mark[i] == 1) return;     // cycle — break it (C structs can't truly cycle by-value)
        mark[i] = 1;
        for (auto d : deps[i]) self(self, d);
        mark[i] = 2;
        order.push_back(i);
    };
    for (std::size_t i = 0; i < tree.records.size(); ++i) visit(visit, i);
    ...
    for (const auto &t : tree.typedefs) {
        ...
        if (it == pm.typedef_to_ctypes.end()) continue;
        if (it->second == t.name) continue;
    ...
    for (const auto &f : tree.functions) {
        ...
        for (const auto &a : f.arg_types) out << ", " << pm.lookup(a);
        out << ")\n";
    }
}
...
void emit_tree(const api_tree &tree, const std::string &path)
{
    ...
    out << "inline constexpr std::size_t record_count   = "
        << tree.records.size()   << ";\n";
    out << "inline constexpr std::size_t enum_count     = "
        << tree.enums.size()     << ";\n";
    ...
    for (std::size_t i = 0; i < tree.records.size(); ++i) {
        ...
        out << "inline constexpr record_info<" << r.fields.size()
            << "> rec_" << i << " = {\n"
            << "    \"" << esc(r.name) << "\",\n"
    ...
    for (std::size_t i = 0; i < tree.enums.size(); ++i) {
        ...
        out << "inline constexpr enum_info<" << e.values.size()
            << "> enm_" << i << " = {\n"
            << "    \"" << esc(e.name) << "\",\n"
            << "    \"" << esc(e.underlying) << "\",\n"
            << "    " << e.size_bytes << ",\n"
...
bool parse_cli(int argc, char **argv, cli_args &out)
{
    for (int i = 1; i < argc; ++i) {
        ...
        if      (a == "--header")  { auto v = next(); if (!v) return false; out.header     = v; }
        else if (a == "--python")  { auto v = next(); if (!v) return false; out.python_out = v; }
        else if (a == "--tree")    { auto v = next(); if (!v) return false; out.tree_out   = v; }
        else {
    ...
    if (out.header.empty()) {
        std::fprintf(stderr,
            "usage: wgpu_introspect --header H.h [--python out.py] [--tree out.hpp]\n");
        return false;
    }
    return true;
}
...
int main(int argc, char **argv)
{
    ...
    for (unsigned i = 0; i < ndiag && shown < 5; ++i) {
        ...
        if (clang_getDiagnosticSeverity(d) >= CXDiagnostic_Error) {
            cx_string_guard s(clang_formatDiagnostic(
    ...
    if (!args.python_out.empty()) emit_python(tree, args.python_out);
    if (!args.tree_out.empty())   emit_tree(tree, args.tree_out);
