...
// Example: specialized handler that does something different for strings
template <>
struct storage_handler<std::string> {
    constexpr void handle(const std::string& value) {
        stored_value = "Processed: " + value;
    }
    
    constexpr std::string retrieve() const {
        return stored_value;
    }
...
constexpr int test() {
    using my_types = type_set<int, double, char, Test>;
    ...
    auto t = handler.get_handler<Test>().allocate();
