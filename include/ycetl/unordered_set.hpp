...
private:
    ...
    constexpr size_type find_slot(const Key &k) const {
        ...
        for (size_type probe = 0; probe < cap; ++probe) {
            ...
            if (s.state == slot_state::filled && _eq(s.key, k)) return i;
        }
        return size_type(-1);
    }
...
public:
    ...
    constexpr bool contains(const Key &k) const { return find_slot(k) != size_type(-1); }
