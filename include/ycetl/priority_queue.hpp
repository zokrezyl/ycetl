...
constexpr void sift_down(C &c, std::size_t i, std::size_t n, Cmp &cmp) {
  for (;;) {
    ...
    std::size_t r = 2 * i + 2;
    std::size_t best = i;
    if (l < n && cmp(c[best], c[l]))
      best = l;
    if (r < n && cmp(c[best], c[r]))
      best = r;
    if (best == i)
      break;
...
template <typename T, typename Container = dynamic_array<T>,
...
public:
  ...
  template <typename Memory>
  constexpr priority_queue(Memory &m, const Compare &cmp) : _c(m), _cmp(cmp) {}
