...
private:
    ...
    static constexpr size_type word_count =
    ...
    static constexpr word_type tail_mask() {
        if (N == 0 || N % bits_per_word == 0) return ~word_type{0};
        return (word_type{1} << (N % bits_per_word)) - 1;
    }

    constexpr void clear_tail() {
        if constexpr (word_count > 0) _words[word_count - 1] &= tail_mask();
    }
...
public:
    ...
    constexpr bitset(word_type v) {
        if constexpr (word_count > 0) _words[0] = v;
        clear_tail();
    }
    ...
    constexpr bitset &set(size_type i, bool v = true) {
        ...
        if (v) _words[i / bits_per_word] |=  mask;
        else   _words[i / bits_per_word] &= ~mask;
        return *this;
    }
    constexpr bitset &set() {
        for (auto &w : _words) w = ~word_type{0};
        clear_tail();
        return *this;
    }
    ...
    constexpr bool any()  const noexcept {
        for (auto w : _words) if (w) return true;
        return false;
    }
    ...
    constexpr bitset &operator&=(const bitset &o) {
        for (size_type i = 0; i < storage_count; ++i) _words[i] &= o._words[i];
        return *this;
    }
    constexpr bitset &operator|=(const bitset &o) {
        for (size_type i = 0; i < storage_count; ++i) _words[i] |= o._words[i];
    ...
    friend constexpr bitset operator&(bitset a, const bitset &b) { return a &= b; }
    friend constexpr bitset operator|(bitset a, const bitset &b) { return a |= b; }
    friend constexpr bitset operator^(bitset a, const bitset &b) { return a ^= b; }
    ...
    constexpr bool operator==(const bitset &o) const {
        for (size_type i = 0; i < storage_count; ++i)
            if (_words[i] != o._words[i]) return false;
        return true;
    }
};
