...
// Open-addressing hash multiset. Same slot/tombstone/rehash machinery
// as ycetl::unordered_set, but every insert unconditionally lands in a
// fresh slot (never returns "already present"), and count/erase walk
// the full probe sequence collecting every match.
...
private:
    ...
    struct slot { slot_state state = slot_state::empty; Key key{}; };
        ...
        for (size_type i = 0; i < _slots.size(); ++i) {
            ...
            for (size_type p = 0; p < new_cap; ++p) {
                ...
                if (ns[j].state == slot_state::empty) {
                    ns[j].key   = std::move(_slots[i].key);
    ...
    template <bool IsConst> class basic_iter {
        ...
        using container_pointer =
            std::conditional_t<IsConst, const unordered_multiset *, unordered_multiset *>;
        container_pointer _s = nullptr;
        size_type _i = 0;
        constexpr void advance_to_filled() {
            while (_s && _i < _s->_slots.size()
                && _s->_slots[_i].state != slot_state::filled) ++_i;
        }
    public:
        using value_type     = Key;
        ...
        constexpr operator basic_iter<true>() const requires(!IsConst) {
            basic_iter<true> r; r._s = _s; r._i = _i; return r;
        }
        ...
        constexpr bool operator==(const basic_iter &o) const { return _s == o._s && _i == o._i; }
...
public:
    ...
    constexpr iterator find(const Key &k) {
        ...
        for (size_type p = 0; p < cap; ++p) {
            ...
            if (_slots[i].state == slot_state::filled && _eq(_slots[i].key, k))
...
private:
    template <typename K>
    constexpr iterator insert_impl(K &&k) {
        maybe_grow();
        size_type cap = _slots.size(), h = _hasher(k) % cap;
        for (size_type p = 0; p < cap; ++p) {
            size_type i = (h + p) % cap;
            if (_slots[i].state != slot_state::filled) {
                _slots[i].key   = std::forward<K>(k);
...
public:
    constexpr iterator insert(const Key &k) { return insert_impl(k); }
    constexpr iterator insert(Key &&k)      { return insert_impl(std::move(k)); }
    ...
    // Removes EVERY occurrence of k. Returns the count.
