...
private:
  ...
  template <bool IsConst> class basic_iter {
    ...
    using container_pointer =
        std::conditional_t<IsConst, const unordered_multiset *,
                           unordered_multiset *>;
...
public:
  ...
  constexpr iterator find(const Key &k) {
    ...
    for (size_type p = 0; p < cap; ++p) {
      size_type i = (h + p) % cap;
      if (_slots[i].state == slot_state::empty)
        return end();
...
private:
  template <typename K> constexpr iterator insert_impl(K &&k) {
    maybe_grow();
    size_type cap = _slots.size(), h = _hasher(k) % cap;
    for (size_type p = 0; p < cap; ++p) {
      size_type i = (h + p) % cap;
...
public:
  ...
  template <typename... Args> constexpr iterator emplace(Args &&...args) {
