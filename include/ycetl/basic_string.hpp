...
public:
  ...
  template <typename Memory>
  constexpr basic_string(Memory &m, const CharT *s) : _data(m) {
    append_raw(s, len_of(s));
  }
  ...
  constexpr basic_string &operator+=(std::basic_string_view<CharT> sv) {
    return append(sv);
  }
