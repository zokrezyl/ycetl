...
private:
  template <typename U>
  static auto test(int) -> decltype(void(U()), std::true_type{});
...
template <typename T, typename Alloc> struct has_allocator_constructor {
private:
  template <typename U, typename A>
  static auto test(int)
      -> decltype(void(U(std::declval<A>())), std::true_type{});
  template <typename, typename> static auto test(...) -> std::false_type;

public:
  static constexpr bool value = decltype(test<T, Alloc>(0))::value;
};
...
template <typename _Tp, typename _Alloc> struct _vector_base {
  typedef typename ycetl::allocator_traits<_Alloc>::template rebind_alloc<_Tp>
      _Tp_alloc_type;
  typedef typename ycetl::allocator_traits<_Tp_alloc_type>::pointer pointer;
...
protected:
  constexpr pointer _allocate(size_t _n) {
    ...
    typedef ycetl::allocator_traits<_Tp_alloc_type> _AllocTraits;
...
class vector : public _vector_base<_Tp, _Alloc> {
  ...
  typedef ycetl::allocator_traits<_tp_alloc_type> _Alloc_traits;
...
public:
  ...
  constexpr vector &operator=(const vector &__x) noexcept(
      ycetl::allocator_traits<
          _Alloc>::propagate_on_container_copy_assignment::value ||
      ycetl::allocator_traits<_Alloc>::is_always_equal::value) {
...
private:
  ...
  static constexpr size_type _max_size(const _tp_alloc_type &__a) noexcept {
    ...
    // (even if ycetl::allocator_traits::max_size says we can).
