...
suite dynamic_array_suite = [] {
  ...
  "size_constructor"_test = [] {
    auto test_lambda = [] {
      ...
      ycetl::dynamic_array<int> arr(alloc, 5);
  ...
  "initializer_list_constructor"_test = [] {
    auto test_lambda = [] {
      test_allocator<int> alloc{};
      ycetl::dynamic_array<int> arr(alloc, {10, 20, 30});
  ...
  "push_back_nontrivial"_test = [] {
    auto test_lambda = [] {
      test_allocator<NonTrivial> alloc{};
      ycetl::dynamic_array<NonTrivial> arr(alloc);
  ...
  "insert_middle"_test = [] {
    auto test_lambda = [] {
      test_allocator<int> alloc{};
      ycetl::dynamic_array<int> arr(alloc, {1, 2, 4});
  ...
  "resize_grow"_test = [] {
    auto test_lambda = [] {
      test_allocator<int> alloc{};
      ycetl::dynamic_array<int> arr(alloc, {1, 2});
      arr.resize(alloc, 5);
  ...
  "clear_and_deallocate_buffer"_test = [] {
    auto test_lambda = [] {
      test_allocator<int> alloc{};
      ycetl::dynamic_array<int> arr(alloc, {1, 2, 3});
      arr.clear_and_deallocate_buffer(alloc);
      return arr.size() == 0_u && arr.capacity() == 0_u &&
             arr.begin() == nullptr;
    };
  ...
  "multitype_memory_usage"_test = [] {
    auto test_lambda = [] {
      using memory_t =
          mem::multitype_memory<mem::dynamic_memory, type_set<int, double>>;
      memory_t memory;
