...
suite unordered_multimap_suite = [] {
    ...
    "iterate_sees_every_pair"_test = [] {
        constexpr auto t = [] {
            unordered_multimap<int, int> m;
            for (int i = 0; i < 5; ++i) m.insert({7, i});
            int sum = 0, n = 0;
            for (auto &kv : m) { sum += kv.second; ++n; }
    ...
    "erase_drops_all_matching_keys"_test = [] {
        constexpr auto t = [] {
            unordered_multimap<int, int> m;
            m.insert({1, 1});
            for (int i = 0; i < 3; ++i) m.insert({9, i});
            m.insert({2, 2});
            return m.erase(9) == 3 && m.size() == 2
                && !m.contains(9) && m.contains(1) && m.contains(2);
        };
        static_assert(t());
        expect(t());
    };
