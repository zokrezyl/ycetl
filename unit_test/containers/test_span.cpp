...
suite span_suite = [] {
  ...
  "from_c_array"_test = [] {
    constexpr auto t = [] {
      int arr[]{1, 2, 3, 4, 5};
      span<int> s(arr);
      return s.size() == 5_u && s.front() == 1 && s.back() == 5 && s[2] == 3
          && !s.empty() && s.size_bytes() == 5 * sizeof(int);
    };
    static_assert(t());
    expect(t());
  };
  ...
  "subspan"_test = [] {
    constexpr auto t = [] {
      ...
      return head.size() == 2_u && head[1] == 1 && tail.size() == 2_u
          && tail[0] == 4 && tail[1] == 5 && mid.size() == 3_u && mid[0] == 1
          && mid[2] == 3;
    };
    static_assert(t());
    expect(t());
