...
suite unordered_multiset_suite = [] {
    "insert_keeps_duplicates"_test = [] {
        constexpr auto t = [] {
            ...
            s.insert(7); s.insert(7); s.insert(7);
            s.insert(1);
    ...
    "erase_removes_all_matching"_test = [] {
        constexpr auto t = [] {
            unordered_multiset<int> s;
            s.insert(1);
            for (int i = 0; i < 4; ++i) s.insert(7);
            s.insert(3);
            return s.erase(7) == 4 && s.size() == 2
                && !s.contains(7) && s.contains(1) && s.contains(3)
                && s.erase(999) == 0;
        };
        static_assert(t());
        expect(t());
    ...
    "rehash_preserves_duplicates"_test = [] {
        constexpr auto t = [] {
            unordered_multiset<int> s;
            for (int i = 0; i < 50; ++i) { s.insert(5); s.insert(i); }
            return s.size() == 100 && s.count(5) == 51 // 50 copies + key=5 from the i-loop
                && s.bucket_count() >= 128;
        };
        static_assert(t());
        expect(t());
