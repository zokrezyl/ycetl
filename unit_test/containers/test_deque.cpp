...
suite deque_suite = [] {
    ...
    "push_front_then_back"_test = [] {
        ...
        constexpr auto t = [] {
            deque<int> d;
            for (int i : {1, 2, 3}) d.push_front(i);  // 3, 2, 1
            for (int i : {4, 5, 6}) d.push_back(i);   // 3, 2, 1, 4, 5, 6
            int seen[6]{};
            for (int i = 0; i < 6; ++i) seen[i] = d[i];
            return seen[0] == 3 && seen[1] == 2 && seen[2] == 1
                && seen[3] == 4 && seen[4] == 5 && seen[5] == 6
                && d.size() == 6;
        };
        static_assert(t());
        expect(t());
    ...
    "growth_under_mixed_push"_test = [] {
        ...
        constexpr auto t = [] {
            ...
            int seen_xor = 0;
            for (auto v : d) seen_xor ^= v;
            int expected = 0;
            for (int i = 0; i < 50; ++i) expected ^= i;
    ...
    "iterate_in_order"_test = [] {
        constexpr auto t = [] {
            deque<int> d;
            for (int i : {1, 2, 3, 4, 5}) d.push_back(i);
            int prev = 0, count = 0;
            for (auto v : d) { if (v != prev + 1) return false; prev = v; ++count; }
            return count == 5;
        };
        static_assert(t());
        expect(t());
